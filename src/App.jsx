...
export default function App() {
  ...
  if (showBrowse) return <BrowseListings onBack={() => setShowBrowse(false)} onPost={() => { setShowBrowse(false); setShowPost(true) }} currentUser={currentUser} initialModal={newListingForModal} onModalClear={() => setNewListingForModal(null)} darkMode={darkMode} onToggleDark={toggleDark} />
  ...
  const T = {
    bg: darkMode ? '#0f0f11' : '#fff',
    bg2: darkMode ? '#141416' : '#f5f5f7',
    cardBg: darkMode ? '#1c1c1e' : '#fff',
    text: darkMode ? '#f5f5f7' : '#1d1d1f',
    textSub: darkMode ? '#8e8e93' : '#6e6e73',
    textFaint: darkMode ? '#636366' : '#aeaeb2',
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; background: ${T.bg}; color: ${T.text}; -webkit-font-smoothing: antialiased; overflow-x: hidden; }
        ...
        .nav-link { color: ${T.textSub}; font-size: 14px; transition: color 0.2s; }
        ...
        .btn-outline { background: transparent; color: ${darkMode ? '#FFCB05' : '#00274C'}; border: 1.5px solid ${darkMode ? 'rgba(255,203,5,0.35)' : 'rgba(0,39,76,0.2)'}; border-radius: 980px; padding: 10px 22px; font-size: 14px; font-weight: 500; transition: all 0.2s; }
        .btn-outline:hover { border-color: ${darkMode ? '#FFCB05' : '#00274C'}; background: ${darkMode ? 'rgba(255,203,5,0.08)' : 'rgba(0,39,76,0.04)'}; }

        .card-hover { transition: all 0.3s ease; }
        .card-hover:hover { box-shadow: 0 16px 48px ${darkMode ? 'rgba(0,0,0,0.4)' : 'rgba(0,39,76,0.13)'}; transform: translateY(-4px); }

        .pill { font-size: 12px; color: ${T.textSub}; background: ${T.bg2}; padding: 4px 11px; border-radius: 980px; }
        .pill-gold { background: rgba(255,203,5,0.15); color: ${darkMode ? '#FFCB05' : '#7a5c00'}; font-weight: 500; }
        ...
        .dark-toggle { width: 36px; height: 36px; border-radius: 50%; border: 1.5px solid ${darkMode ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.1)'}; background: ${darkMode ? 'rgba(255,255,255,0.08)' : '#f5f5f7'}; cursor: pointer; font-size: 15px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.2s; }
      ...
      <nav style={{
        ...
        background: scrolled ? (darkMode ? 'rgba(15,15,17,0.92)' : 'rgba(255,255,255,0.9)') : 'transparent',
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(20px, 4vw, 48px)', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {currentUser ? (
            <div className="nav-desktop-mid" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <button onClick={() => setShowPost(true)} style={{ background: '#FFCB05', color: '#00274C', border: 'none', borderRadius: 980, padding: '10px 22px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s', boxShadow: '0 2px 12px rgba(255,203,5,0.3)' }}
                onMouseEnter={e => { e.target.style.background='#ffe033'; e.target.style.transform='translateY(-1px)' }}
                onMouseLeave={e => { e.target.style.background='#FFCB05'; e.target.style.transform='none' }}>
                Post a Listing
              </button>
            </div>
          ) : (
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            {currentUser ? (
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#00274C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#FFCB05', cursor: 'pointer', flexShrink: 0, boxShadow: '0 2px 8px rgba(0,39,76,0.25)' }} onClick={() => setShowDashboard(true)}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(20px, 4vw, 48px)', width: '100%' }}>
          <div className="hero-inner" style={{ display: 'flex', alignItems: 'center', gap: 72 }}>
            <div style={{ flex: 1, opacity: mounted ? 1 : 0, transform: mounted ? 'none' : 'translateY(28px)', transition: 'all 0.9s cubic-bezier(0.4,0,0.2,1)' }}>
              ...
              <p style={{ fontSize: 19, lineHeight: 1.65, color: T.textSub, marginBottom: 40, maxWidth: 460, fontWeight: 400 }}>
              ...
              <div style={{ display: 'flex', gap: 0, borderTop: `1px solid ${T.border}`, paddingTop: 32 }}>
                {[['100%', 'UMich verified'], ['$0', 'Zero fees'], ['2 min', 'To post']].map(([v, l], i) => (
                  <div key={l} style={{ flex: 1, paddingRight: 24, borderRight: i < 2 ? `1px solid ${T.border}` : 'none', paddingLeft: i > 0 ? 24 : 0 }}>
                    <div style={{ fontSize: 28, fontWeight: 800, color: darkMode ? '#FFCB05' : '#00274C', letterSpacing: '-0.03em' }}>{v}</div>
                    <div style={{ fontSize: 13, color: T.textFaint, marginTop: 3 }}>{l}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="hero-card-col float-card" style={{ width: 360, flexShrink: 0, opacity: mounted ? 1 : 0, transition: 'opacity 1s ease 0.3s' }}>
              <div style={{ background: T.cardBg, borderRadius: 24, border: `1.5px solid ${T.border}`, boxShadow: `0 40px 100px ${darkMode ? 'rgba(0,0,0,0.5)' : 'rgba(0,39,76,0.14)'}`, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 20 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: T.text, marginBottom: 3 }}>{listing.title}</div>
                      <div style={{ fontSize: 12, color: T.textFaint }}>{listing.location}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: 22, fontWeight: 800, color: darkMode ? '#FFCB05' : '#00274C' }}>${listing.price}</span>
                      <span style={{ fontSize: 12, color: T.textFaint }}>/mo</span>
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 7, paddingBottom: 18 }}>
                  {LISTINGS.map((_, i) => (
                    <button key={i} style={{ width: i === activeCard ? 22 : 7, height: 7, borderRadius: 980, background: i === activeCard ? '#00274C' : (darkMode ? '#3a3a3e' : '#e5e7eb'), border: 'none', cursor: 'pointer', padding: 0, transition: 'all 0.3s' }} onClick={() => setActiveCard(i)} />
                  ))}
                </div>
              </div>
              <div style={{ textAlign: 'center', marginTop: 14, fontSize: 12, color: T.textFaint }}>40+ active listings this semester</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(20px, 4vw, 48px)' }}>
          ...
          <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {[
              ...
              <div key={p.title} data-section={`prob${i}`} className={`card-hover section-fade stagger-${i + 1} ${vis(`prob${i}`) ? 'visible' : ''}`} style={{ background: T.cardBg, borderRadius: 20, padding: '36px 32px', border: `1px solid ${T.border}`, boxShadow: `0 2px 8px ${darkMode ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.04)'}` }}>
                <div style={{ fontSize: 44, marginBottom: 20 }}>{p.icon}</div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: T.text, marginBottom: 12, letterSpacing: '-0.01em' }}>{p.title}</h3>
                <p style={{ fontSize: 15, color: T.textSub, lineHeight: 1.65 }}>{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: T.bg, padding: '110px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(20px, 4vw, 48px)' }}>
          <div data-section="solution" className={`section-fade ${vis('solution') ? 'visible' : ''}`} style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <p style={{ fontSize: 17, color: T.textSub, marginTop: 16, maxWidth: 500, margin: '16px auto 0' }}>Built from scratch for Wolverines, by Wolverines.</p>
          </div>
          <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {[
              ...
              <div key={f.title} data-section={`feat${i}`} className={`card-hover section-fade stagger-${i + 1} ${vis(`feat${i}`) ? 'visible' : ''}`} style={{ background: T.cardBg, borderRadius: 20, padding: '36px 32px', border: `1px solid ${T.border}`, boxShadow: `0 2px 8px ${darkMode ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.04)'}` }}>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 700, color: T.text, marginBottom: 12, letterSpacing: '-0.01em' }}>{f.title}</h3>
                <p style={{ fontSize: 15, color: T.textSub, lineHeight: 1.65 }}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: T.bg2, padding: '110px 0' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 clamp(20px, 4vw, 48px)', textAlign: 'center' }}>
          <div data-section="preview" className={`section-fade ${vis('preview') ? 'visible' : ''}`}>
            ...
            <p style={{ fontSize: 17, color: T.textSub, marginBottom: 56 }}>Clean, fast, and exactly what you need.</p>
          </div>
          <div data-section="mockup" className={`section-fade ${vis('mockup') ? 'visible' : ''}`} style={{ background: T.cardBg, borderRadius: 20, padding: 8, boxShadow: `0 40px 100px ${darkMode ? 'rgba(0,0,0,0.4)' : 'rgba(0,0,0,0.1)'}`, border: `1px solid ${T.border}` }}>
            <div style={{ background: T.bg2, borderRadius: 13, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${T.border}` }}>
                <div style={{ display: 'flex', gap: 6 }}>
                  {['#ff5f57', '#febc2e', '#28c840'].map(c => <div key={c} style={{ width: 11, height: 11, borderRadius: '50%', background: c }} />)}
                </div>
                <div style={{ flex: 1, background: T.cardBg, borderRadius: 7, padding: '5px 12px', fontSize: 12, color: T.textFaint, textAlign: 'center', border: `1px solid ${T.border}` }}>subly.app/browse</div>
              </div>
              <div style={{ padding: '20px 24px' }}>
                <div style={{ background: T.cardBg, border: `1.5px solid ${T.border}`, borderRadius: 12, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <span style={{ fontSize: 14, color: T.textFaint }}>🔍</span>
                  <span style={{ fontSize: 13, color: T.textFaint }}>Search by dates, price, or neighborhood...</span>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {['May to August', 'Under $1,200', 'Studio / 1BR', 'Near Central Campus'].map((f, i) => (
                    <div key={f} style={{ fontSize: 12, padding: '5px 12px', borderRadius: 980, background: i === 0 ? '#00274C' : T.cardBg, color: i === 0 ? '#FFCB05' : T.textSub, border: `1px solid ${i === 0 ? '#00274C' : T.border}`, fontWeight: i === 0 ? 600 : 400 }}>{f}</div>
                  ))}
                </div>
                <div className="preview-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                  {LISTINGS.map((l, i) => (
                    <div key={l.id} className="card-hover" style={{ background: T.cardBg, borderRadius: 12, padding: 16, border: `1px solid ${T.border}` }}>
                      ...
                      <div style={{ fontSize: 13, fontWeight: 700, color: T.text, marginBottom: 3 }}>{l.title}</div>
                      <div style={{ fontSize: 11, color: T.textFaint, marginBottom: 8 }}>{l.dates}</div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(20px, 4vw, 48px)' }}>
          ...
          <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginBottom: 32 }}>
            {[
              ...
              <div key={step.n} data-section={`step${i}`} className={`section-fade stagger-${i + 1} ${vis(`step${i}`) ? 'visible' : ''}`} style={{ borderRadius: 20, padding: '40px 32px', border: '1.5px solid', textAlign: 'center', transition: 'all 0.4s ease', borderColor: activeStep === i ? '#FFCB05' : T.border, background: activeStep === i ? (darkMode ? 'rgba(255,203,5,0.05)' : 'rgba(0,39,76,0.03)') : T.cardBg, boxShadow: activeStep === i ? `0 8px 32px ${darkMode ? 'rgba(0,0,0,0.3)' : 'rgba(0,39,76,0.1)'}` : 'none' }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: T.textFaint, letterSpacing: '0.06em', marginBottom: 20 }}>{step.n}</div>
                <div style={{ fontSize: 44, marginBottom: 20 }}>{step.icon}</div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: T.text, marginBottom: 12, letterSpacing: '-0.01em' }}>{step.title}</h3>
                <p style={{ fontSize: 15, color: T.textSub, lineHeight: 1.65 }}>{step.desc}</p>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
            {[0, 1, 2].map(i => (
              <button key={i} onClick={() => setActiveStep(i)} style={{ width: i === activeStep ? 22 : 7, height: 7, borderRadius: 980, background: i === activeStep ? '#00274C' : (darkMode ? '#3a3a3e' : '#d2d2d7'), border: 'none', cursor: 'pointer', padding: 0, transition: 'all 0.3s' }} />
            ))}
          </div>
        </div>
      </section>
